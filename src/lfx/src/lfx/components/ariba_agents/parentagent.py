...
class ParentagentComponent(Component):
    display_name: str = "Parentagent"
    description: str = "Parentagent - A specialized Ariba agent for parent-level coordination and management tasks."
    icon: str = "bot"
    name: str = "Parentagent"
    ...
    inputs = [
        ...
        StrInput(
            ...
            value="Parent-level coordination and management",
            advanced=False,
        ),
        MultilineInput(
            ...
            value="You are Parentagent, a specialized agent for parent-level coordination and management tasks.",
            advanced=False,
        ),
        IntInput(
        ...
        MessageTextInput(
            ...
            info="The message or task to send to the Parentagent.",
            tool_mode=True,
        ),
        MultilineInput(
    ...
    AGENT_ID = "7c034ba2-9492-4d18-8e79-b52dc27bbc80"
            ...
            async with httpx.AsyncClient() as client:
                ...
                if agent_data:
                    ...
                    if "agent_name" in build_config:
                        build_config["agent_name"]["value"] = agent_data.get("name", "Parentagent")
                    
                    if "expertise" in build_config:
                        build_config["expertise"]["value"] = agent_data.get("expertIn", "Parent-level coordination and management")
                    
                    if "initial_instructions" in build_config:
                        build_config["initial_instructions"]["value"] = agent_data.get("initialInstructions", "You are Parentagent, a specialized agent for parent-level coordination and management tasks.")
    ...
    async def message_response(self) -> Message:
        """Execute the Parentagent and return the response as a Message."""
        if not self.input_value:
            return Message(
                text="Please provide a message or task for the Parentagent to execute.",
                sender="Parentagent",
                sender_name="Parentagent",
            )
        ...
        try:
            ...
            error_msg = f"Error executing Parentagent: {e!s}"
            return Message(
                text=error_msg,
                sender="Parentagent",
                sender_name="Parentagent",
